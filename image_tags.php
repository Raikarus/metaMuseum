...
$(document).ready(function(){
    ...
    function load_page()
    {
      ...
      {
        // Добавить стили
        $('#wrapping').html('<button id = "download" style="border-radius:10px;width:100px;height:50px;font-size:20px;border:1px solid white;background-color:black;margin-top:10px;margin-left:10px;color:white;cursor:pointer">Загрузить</button>');
      }
    }
    ...
    $('#link_tags').click(function(){
      ...
      data = {'action':'link_kword','img_names':img_names,'new_kwords':new_kwords,'delete_kwords':delete_kwords,'auto_kwords':auto_kwords};
      $.post(ajaxurl,data).done(function(response)  {
        console.log(response);
